...
function PantallaClientes() {
  ...
  const guardarEnHistorial = async () => {
    ...
    try {
      ...
      // Resetear checks: limpiar hora_llegada en Supabase y estado local
      const idsConCheck = Object.keys(checksHora).map(Number).filter(Boolean);
      if (idsConCheck.length > 0) {
        await supabase.from('historial_clientes')
          .update({ hora_llegada: null })
          .eq('fecha', hoy)
          .in('cliente_id', idsConCheck);
      }
      setChecksHora({});
                      ...
                      <div className="flex justify-center items-center gap-3">
                        {cliente.chofer && cliente.Choferes?.celular && (
                          <button
                            ...
                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#25d366', display: 'flex', padding: '4px', borderRadius: '6px', transition: 'all 120ms' }}
                            onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#25d36620'; }}
                        ...
                        <button
                          ...
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef444470', display: 'flex', padding: '4px', borderRadius: '6px', transition: 'all 120ms' }}
                          onMouseEnter={e => { e.currentTarget.style.color = '#ef4444'; e.currentTarget.style.backgroundColor = '#ef444415'; }}
                          onMouseLeave={e => { e.currentTarget.style.color = '#ef444470'; e.currentTarget.style.backgroundColor = 'transparent'; }}
                        >
                          <Trash2 size={18} />
                        </button>
                      </div>
